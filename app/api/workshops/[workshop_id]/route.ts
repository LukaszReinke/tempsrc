...
const authRequired = true;
  ...
  export async function PUT(req: NextRequest, { params }: { params: Promise<{ workshop_id: string }> }) {
    const { workshop_id } = await params;
    if (!workshop_id) {
      return NextResponse.json({ error: 'Workshop ID is required' }, { status: 400 });
    }
  ...
  export async function DELETE(req: NextRequest, { params }: { params: Promise<{ workshop_id: string }> }) {
    const { workshop_id } = await params;
    if (!workshop_id) {
      return NextResponse.json({ error: 'Workshop ID is required' }, { status: 400 });
    }
  ...
  export async function PATCH(req: NextRequest, { params }: { params: Promise<{ workshop_id: string }> }) {
    const { workshop_id } = await params;
    if (!workshop_id) {
      return NextResponse.json({ error: 'Workshop ID is required' }, { status: 400 });
    }
    try {
      const workshop = await apiClient(`workshops/${workshop_id}/approve`, { method: 'GET', authRequired });
      return NextResponse.json(workshop);
    } catch (error) {
      console.error('Error fetching workshop:', error);
