...
const authRequired = true;
  ...
  export async function PUT(req: NextRequest, { params }: { params: Promise<{ contest_id: string }> }) {
    const { contest_id } = await params;
    if (!contest_id) {
      return NextResponse.json({ error: 'Contest ID is required' }, { status: 400 });
    }
  ...
  export async function DELETE(req: NextRequest, { params }: { params: Promise<{ contest_id: string }> }) {
    const { contest_id } = await params;
    if (!contest_id) {
      return NextResponse.json({ error: 'Contest ID is required' }, { status: 400 });
    }
  ...
  export async function PATCH(req: NextRequest, { params }: { params: Promise<{ contest_id: string }> }) {
    const { contest_id } = await params;
    if (!contest_id) {
      return NextResponse.json({ error: 'Contest ID is required' }, { status: 400 });
    }
